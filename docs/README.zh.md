...
真拆开了看，一个 Agent 不过就是：一个大模型、一个工具循环、一点状态管理。几百行 Python，够了。

## 为什么不用现成的框架？

如果你要快速交付产品，用框架。Bryx 适合的场景是：你想*搞懂*一个 Agent 怎么运作——工具调用为什么挂了，不用翻六层抽象去查；想改主循环的行为，不用先啃两万行别人的代码。
...
| 模块 | 状态 |
|---|---|
| LLM 客户端 — Anthropic SDK 封装，流式、thinking、token 统计 | ✅ |
| 工具注册表 — 注册 handler，导出 schema，异步调度 | ✅ |
...
| CLI REPL / HTTP API | 📋 |
| 自进化循环 — propose → apply → evaluate，每次实验一个分支 | 📋 |

第一个里程碑：把我之前写的另一个 Agent 的核心能力，手写复刻一遍。从一个能真用的基线开始，不搞玩具 demo。
...
只要是 Anthropic 协议兼容的网关都能用——Claude、DeepSeek 网关等等。改一下 `ANTHROPIC_BASE_URL` 就行。
